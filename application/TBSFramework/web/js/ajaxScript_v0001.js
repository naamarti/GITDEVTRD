...
// ------------------------------------------------------------------
//
// Val Catrini, added code below here
//
// ------------------------------------------------------------------

var ajaxScript					= new Object();
ajaxScript.callbackMethod		= null;
ajaxScript.response				= null;
ajaxScript.responseType			= "";

ajaxScript.getXML = function(url, parameters, callbackMethod)
{
	ajaxScript.run(url, parameters, callbackMethod, "xml");
}
ajaxScript.getXMLAccountList = function(url, parameters, callbackMethod)
{
	ajaxScript.run(url, parameters, callbackMethod, "xml");
}
ajaxScript.getText = function(url, parameters, callbackMethod)
{
	ajaxScript.run(url, parameters, callbackMethod, "txt");
}

ajaxScript.run = function(url, parameters, callbackMethod, responseType)
{
	ajaxScript.callbackMethod = callbackMethod;
	ajaxScript.responseType = responseType;
	ajaxScript.response = null;
	ajaxpack.postAjaxRequest(url, parameters, ajaxScript.onStateChange, responseType);
}

ajaxScript.onStateChange = function()
{
	var myajax=ajaxpack.ajaxobj
	var myfiletype=ajaxpack.filetype
	...
	{ //if request of file completed
		...
		{ //if request was successful or running script locally
			if(ajaxScript.responseType=="xml")
			{
				ajaxScript.response = myajax.responseXML;   //.documentElement;
			}
			else
			{
				ajaxScript.response = myajax.responseText;
			}				
			ajaxScript.onSuccess();
		}
	}
}

ajaxScript.onSuccess = function()
{
	ajaxScript.callbackMethod();
}

ajaxScript.getXMLElement = function(xmlDoc, elementName)
{
