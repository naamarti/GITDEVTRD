...
import com.totalbanksolutions.trading.web.action.sitedocuments.SiteDocumentsDisplayAction;
import com.totalbanksolutions.trading.web.model.ModelHelper;

/**
 * @author Val Catrini
 */
public class SiteDocumentsController extends AbstractControllerNew 
{
	...
	public void processRequest( HttpServletRequest request, ModelAndView modelAndView ) 
	{
		...
		log.debug("### SiteDocumentsController sessionUserAction = " + sessionUserAction + " ###");

		if( sessionUserAction.length() > 0 )
		{
		    String reportDirName = getUserDocumentPath() + "Site Documents\\";
			...
			catch(Exception e)
			{
				...
				h.setMessageError( "Unexpected error while trying to open PDF report.<br>" + ExceptionUtils.getMessage(e) );
		...
		{
			new SiteDocumentsDisplayAction( this.ds ).processAction( request, modelAndView );
		}
	}
